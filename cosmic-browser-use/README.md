...
- Jev decision engine (default): routine structured-page steps are decided by one ~0.4s TypeSafe System One call over the DOM snapshot's `@e` ref map — no screenshot in the decision, no free-form generation — while escalations, vision pages, low confidence, and every non-routine step fall through to the full LLM planner. Jev steps never self-complete a run (only the DONE finalizer finishes one), see the saved-notes knowledge base, can record notes themselves (`SAVE_NOTE`, composed by the text helper), stand down after repeated fall-throughs, and follow one-line planner guidance (`fast_engine_hint`) when the planner had to take over a step. Toggle with `--decision-engine jev|llm`; auto-downgrades to the classic planner without `TYPESAFE_API_KEY` or in vision mode.
