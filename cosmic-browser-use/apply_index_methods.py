#!/usr/bin/env python3
"""
Apply index-based retrieval methods to browser_controller.py
"""
import re

# Read the file
with open(r"c:\Users\Praveen Raj U S\CBU\cosmic-browser-use\browser_controller.py", "r", encoding="utf-8") as f:
    content = f.read()

# Method 1: _get_large_note_by_id
new_get_method = '''    def _get_large_note_by_id(self, note_id: str) -> Optional[Dict[str, Any]]:
...
pattern1 = r'    def _get_large_note_by_id\(self, note_id:.*?\n        return None\n'
content = re.sub(pattern1, new_get_method + '\n', content, flags=re.DOTALL)

# Method 2: _list_large_notes
new_list_method = '''    async def _list_large_notes(self, limit: Any = 20, newest_first: Any = True) -> ActionResult:
...
pattern2 = r'    async def _list_large_notes\(self, limit:.*?(?=\n    async def |\n    def |\nclass )'
content = re.sub(pattern2, new_list_method + '\n', content, flags=re.DOTALL)

# Method 3: _search_large_notes
new_search_method = '''    async def _search_large_notes(self, query: str, limit: Any = 10) -> ActionResult:
...
pattern3 = r'    async def _search_large_notes\(self, query:.*?(?=\n    async def |\n    def |\nclass )'
content = re.sub(pattern3, new_search_method + '\n', content, flags=re.DOTALL)

# Write back
with open(r"c:\Users\Praveen Raj U S\CBU\cosmic-browser-use\browser_controller.py", "w", encoding="utf-8") as f:
    f.write(content)

print("✅ Applied all index-based retrieval methods to browser_controller.py")
