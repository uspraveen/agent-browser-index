...
"""
    ...
    def _build_system_prompt(self, context: Dict[str, Any]) -> str:
        ...
        if dom_enabled:
            available_tools_definitions[4:4] = [
                ...
                "- SelectOption(selector, value|label|index, values|labels for multi-select) - Select option(s) in a native <select> dropdown (DETERMINISTIC — use this for DETECTED DROPDOWNS instead of clicking invisible options). Fires real change events, works in iframes and shadow DOM. The selector must match exactly ONE visible dropdown — ambiguous selectors are refused.",
                "- DomType(selector, text, press_enter) - Type into an input/textarea matched by a CSS selector. Use this instead of VisualType when you already know a stable selector for the field (e.g. from a prior DOMExtract) — it's faster and more reliable than visual grounding. The selector must match exactly ONE visible field: forms on many sites reuse one placeholder (e.g. 'Type here...') across unrelated inputs, and an ambiguous selector is REFUSED rather than resolved to the first match, because guessing can type one field's value into another. Every successful DomType echoes back the label of the field that received the text — read it.",
                "- DOMExtract(query) - Extract text/data from DOM. Limit 100k chars. Text links (anchors) come back as 'title — resolved URL' in one entry, so link/URL collection needs a single extract. Prefer semantic selectors: 'main', 'article', '[role=\"main\"]', '.readme', '.model-card', '#content', '.post-body' etc. Only use 'body' if no semantic container exists.",
                ...
                "- DOMSnapshot(max_elements) - Structured perception: ONE call returns every visible interactive element as a numbered map — '@e5 button \"Apply for this Job\"', '@e3 textbox \"Legal Name\" value=...' (passwords masked) — far cheaper than grounding each action visually. Run it after landing on any structured page (forms, lists, paginated boards); act on the refs with SnapshotClick/SnapshotType/SnapshotSelect. Re-snapshot after navigation or big DOM changes; refs are fingerprint-checked and stale ones are refused. Vision and raw selectors remain fully available — the snapshot is usually the fastest accurate first pass, not the only one.",
    ...
    -   *Selector Loop Rule*: Do not run more than 2 DOMExtract attempts for the same information when prior outputs are non-empty. If two attempts return identical output, the selector wording is NOT the problem — the data is not in the text layer you queried. Change what you reach for: text links already carry their resolved href after the title in extract output, so collecting URLs needs no second extract; otherwise open one element, use a different attribute/tool, or SaveNote the partial answer. Rewording the same query a third time is guaranteed to fail the same way."""
...
{escalation_line}
## CORE RULES
1.  **Route by situation — every tool stays available at every step.** Structured pages (forms, lists, paginated boards, dashboards): `DOMSnapshot` + the @ref actions are usually the fastest accurate first pass — one text pull replaces a grounding call per action. No reliable structure (canvas, custom widgets, image-only buttons), or you need to SEE what a change did: use Vision (VisualClick, VisualType). Already know a unique anchor from extraction or markup: raw selectors (DOMClick, DomType, SelectOption) skip straight to the target. Switch routes freely, including mid-task — a snapshot first does not commit you to refs, and vision is never off the table.
{extraction_rule}
{parallel_rule}
